...
namespace core.audiamus.aux {
  public class LogTmpFileMaintenance {
    ...
    public void Cleanup () {
      ...
      var (files, stats) = gather (ApplEnv.TempDirectory);
      var log = gather (ApplEnv.LogDirectory);

      var tmp2 = cleanup (files, stats);
      var log2 = cleanup (log.files, log.stats);

      var tmp3 = cleanup (files, log2?.Timestamp ?? default);
      var log3 = cleanup (log.files, tmp2?.Timestamp ?? default);

      int numFiles = (stats?.NumFiles ?? 0) + (log.stats?.NumFiles ?? 0);
      long totalSize = (stats?.TotalSize ?? 0) + (log.stats?.TotalSize ?? 0);
    ...
    private DirectoryStatistics? cleanup (List<FileInfo> fileInfos, DirectoryStatistics stats) => 
      cleanup (fileInfos, stats, null);
    
    private DirectoryStatistics? cleanup (List<FileInfo>? fileInfos, DateTime enforceByDate) => 
      cleanup (fileInfos, null, enforceByDate);

    private DirectoryStatistics? cleanup (
      List<FileInfo>? fileInfos, 
      DirectoryStatistics? stats, 
      DateTime? enforceByDate
    ) {
    ...
    private static (List<FileInfo> files, DirectoryStatistics stats) gather (string dir) {

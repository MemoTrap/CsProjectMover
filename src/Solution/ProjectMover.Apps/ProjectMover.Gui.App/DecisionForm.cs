...
namespace ProjectMover.Gui.App {
  public partial class DecisionForm : Form {
    ...
    private const string HDR_PROJ =
      "Dependent project roots that shall reference the copied project (will include all interdependencies)";
    private const string HDR_SLN =
      "Related solutions that shall reference the copied project";
    ...
    private void init () {
      ...
      toolTip.SetToolTip (labelCurProjFolder, toolTip.GetToolTip (roTxtBoxCurProjFolder));
      toolTip.SetToolTip (labelNewProjFolder, toolTip.GetToolTip (txtBxNewProjFolder));


      if (_context.CopyModeHint) {
        pathCheckListProj.SetPaths (_context.SelectableDependentProjectRoots, _context.DependentProjectRootsPreset);
        pathCheckListSln.SetPaths (_context.SelectableSolutions, _context.SolutionPreset);
      } else {

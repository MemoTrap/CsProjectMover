namespace ProjectMover.Lib.Api {
  ...
  public record ProjectUserDecision (
    ...
    IReadOnlyCollection<string>? SelectedSolutions,             // copy only: absolute solution paths
    IReadOnlyCollection<string>? SelectedDependentProjectRoots  // copy only: absolute project paths
  ) {
  ...
  public record ProjectDecisionContext (
    ...
    int[]? SolutionPreset,                    // indices of previously selected solutions in SelectableSolutions
    int[]? DependentProjectRootsPreset        // indices of previously selected dependent project roots in SelectableDependentProjectRoots
  );

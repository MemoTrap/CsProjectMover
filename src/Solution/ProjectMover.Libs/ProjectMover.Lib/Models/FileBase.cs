  ...
  internal abstract class FileBase {
    ...
    protected FileBase (string filePath, string extension) {
      ...
      if (!string.Equals (ext, extension, StringComparison.OrdinalIgnoreCase))
        throw new ArgumentException ($"File '{filePath}' must have '{extension}' extension.", nameof (filePath));

      bool rooted = Path.IsPathRooted (filePath);
      if (!rooted)
        throw new ArgumentException ($"Path '{filePath}' must be rooted", nameof (filePath));
    ...
    public override string ToString () => $"{this.GetType().Name}: {OrigName}";

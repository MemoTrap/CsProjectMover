...
namespace ProjectMover.Lib.Models {
  ...
  internal class SolutionFile (string filePath) : FileBase (filePath, EXT) {
    ...
    private List<ProjectLine> ProjectLines =>  SolutionLines
      .OfType<ProjectLine> ()
      .ToList ();

    internal IEnumerable<string> ProjectRelativePaths => ProjectLines
      .Where (pl => pl.AbsolutePath is not null)
      .Select (pl => pl.RelativePath)
      .ToList ();

    public IEnumerable<string> ProjectsAbsPath => ProjectLines
      .Where (pl => pl.AbsolutePath is not null)
    ...
    public bool ContainsProject (string absProjPath) {
      if (!ProjectLines.Any ())
        throw new InvalidOperationException ($"Solution not loaded '{AbsolutePath}'.");

...
namespace ProjectMover.Lib.Models {
  internal class CsProjectFile (string projectFilePath) : ProjectFile(projectFilePath, EXT) {
    ...
    public override async Task LoadAsync (CancellationToken ct = default) {
      ...
      var root = _xml.Root
        ?? throw new InvalidOperationException ($"Invalid project file: '{AbsolutePath}'");
      ...
      if (!IsSdkStyle) {
        ...
        if (guidElement is null || string.IsNullOrWhiteSpace (guidElement.Value))
          throw new InvalidOperationException ($"Legacy C# project has no ProjectGuid: '{AbsolutePath}.");
    ...
    public override async Task<string> SaveExAsync (bool dryRun, CancellationToken ct = default) {
      if (_xml is null)
        throw new InvalidOperationException ($"Project not loaded '{AbsolutePath}'.");
    ...
    private void setOrUpdateProperty (string propertyName, string value) {
      if (_xml is null)
        throw new InvalidOperationException (
            $"Project not loaded '{AbsolutePath}'.");

      var ns = _xml.Root!.Name.Namespace;

      // Update if existing
      var existing =
          _xml.Descendants (ns + propertyName).FirstOrDefault ();
      ...
      // Find non-conditional PropertyGroup
      var pg =
          _xml.Root!
      ...
      var newElement = new XElement (ns + propertyName, value);

      // Determine indentation from existing children (default 4 spaces)
      string indent = "    ";

      var first = pg.Elements ().FirstOrDefault ();
      if (first?.PreviousNode is XText ws) {
        var text = ws.Value;
        var lastNl = text.LastIndexOf ('\n');
        if (lastNl >= 0)
          indent = text[(lastNl + 1)..];
      }

      var trailingWhitespace =
          pg.Nodes ().OfType<XText> ().LastOrDefault ();

      if (trailingWhitespace != null) {
        trailingWhitespace.AddBeforeSelf (
            new XText (Environment.NewLine + indent),
            newElement
        );
      } else {
        pg.Add (

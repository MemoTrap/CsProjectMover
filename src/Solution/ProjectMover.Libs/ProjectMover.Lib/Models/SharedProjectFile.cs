namespace ProjectMover.Lib.Models {
  internal class SharedProjectFile (string projectFilePath) : ProjectFile(projectFilePath, EXT) {
    ...
    private async Task loadShProjAsync (CancellationToken ct) {
      ...
      var root = _shprojXml.Root ?? throw new InvalidOperationException ($"Invalid shproj '{AbsolutePath}'.");
      ...
      if (guidElement is null || string.IsNullOrWhiteSpace (guidElement.Value))
        throw new InvalidOperationException ($"Shared project has no ProjectGuid '{AbsolutePath}'.");
      ...
      var import = root
        ...
        ?? throw new InvalidOperationException ($"Shared project has no .projitems import '{AbsolutePath}'.");
      ...
      if (!File.Exists (_projItemsAbsPath))
        throw new FileNotFoundException ($"projitems not found '{AbsolutePath}'.");
    }
    ...
    private async Task loadProjItemsAsync (CancellationToken ct) {
      ...
      if (sharedGuidElement is null || string.IsNullOrWhiteSpace (sharedGuidElement.Value))
        throw new InvalidOperationException ($"projitems has no SharedGUID '{AbsolutePath}'.");

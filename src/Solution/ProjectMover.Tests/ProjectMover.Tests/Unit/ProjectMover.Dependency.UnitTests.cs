...
namespace ProjectMover.Tests.Unit {
  ...
  public sealed class Tests01_ProjectDependencyGraphBuilder {
    ...
    CollectionAssert.AreEquivalent (
      ...
      var selectionGroups = graph.BuildSelectionGroups (
        aLibCore, ProjectDependencyGraph.ERefKind.ReferencedBy
      );
    ...
    public void DepGraph02_SSharedStuff () {
      ...
      var graph = new ProjectDependencyGraph (
        sharedProj, referencingProjects
      );
      ...
      var selectionGroups = graph.BuildSelectionGroups (
        sharedProj, ProjectDependencyGraph.ERefKind.ReferencedBy);
      var groupRoots = selectionGroups.Select (g => g.Root.Name).ToList ();
      CollectionAssert.AreEquivalent (expectedRefBy, groupRoots);
    ...
    internal static async Task<DependentSolutionsAndProjects> LoadProjectsAndSolutionsAsync (
      TestFieldFixture field
    ) {
      // load all projects from test field
      var zLibStd = await loadCs (field.Project (PROJ_Z_LIB_STD));
      var aLibCore = await loadCs (field.Project (PROJ_A_LIB_CORE));
      var bLibUtil = await loadCs (field.Project (PROJ_B_LIB_UTIL));
      var cLibApp = await loadCs (field.Project (PROJ_C_LIB_APP));
      var dLibPlugin = await loadCs (field.Project (PROJ_D_LIB_PLUGIN));
      var eLibWithLinks = await loadCs (field.Project (PROJ_E_LIB_WITH_LINKS));
      var fSharedApp = await loadCs (field.Project (PROJ_F_SHARED_APP));
      var gMixedApp = await loadCs (field.Project (PROJ_G_MIXED_APP));
      var sSharedStuff = await loadShared (field.Project (PROJ_S_SHARED_STUFF));

      // load all solutions from the test field
      var solCore = await loadSolution (field.Solution (SLN_SOL_CORE));
      var solApp = await loadSolution (field.Solution (SLN_SOL_APP));
      var solPlugin = await loadSolution (field.Solution (SLN_SOL_PLUGIN));
      var solLinks = await loadSolution (field.Solution (SLN_SOL_LINKS));
      var solShared = await loadSolution (field.Solution (SLN_SOL_SHARED));
      var solMixed = await loadSolution (field.Solution (SLN_SOL_MIXED));
    ...
    [TestMethod]
    public void DepGraph03_GMixedApp_Refs () {
      // --- Arrange

      var gMixedApp = _dependencies.Projects
          .First (p => p.Name == G_MIXED_APP);


      // --- Act: build general graph
      var graph = new ProjectDependencyGraph (
        _dependencies.Projects
      );
      ...
      string[] expectedNodes =
          [
            A_LIB_CORE, B_LIB_UTIL, C_LIB_APP, D_LIB_PLUGIN,
            E_LIB_WITH_LINKS, F_SHARED_APP, G_MIXED_APP, S_SHARED_STUFF,
            Z_LIB_STD
          ];
      ...
      // --- Assert: GMixedApp ReferencedBy ---
      var alibNode = nodes[G_MIXED_APP];
      string[] expectedRef =
      [
        A_LIB_CORE, B_LIB_UTIL, D_LIB_PLUGIN, E_LIB_WITH_LINKS, Z_LIB_STD
      ];
      CollectionAssert.AreEquivalent (expectedRef, alibNode.References.Select (n => n.Project.Name).ToList ());


      // --- Assert: selection groups from GMixedApp ---
      var selectionGroups = graph.BuildSelectionGroups (
        gMixedApp, ProjectDependencyGraph.ERefKind.References
      );
      var groupRoots = selectionGroups.Select (g => g.Root.Name).ToList ();
      CollectionAssert.AreEquivalent (expectedRef, groupRoots);
